...
class VectoredRNNInterface():

    def __init__(self, model, buckets, vocab_vectors, model_path=""):
        self.model = model
        ...
        if model.size != vector_size:
            raise Exception("Seq2Seq cell size and embedding vector size have to match ({0} vs {1}).".format(model.size, vector_size))

        self._graph_builded = False
        self.encoder_inputs = []
        self.decoder_inputs = []
        for i in range(self.buckets[-1][0]):  # last bucket is the biggest one.
            self.encoder_inputs.append(tf.placeholder(tf.float32, shape=[self._batch_size, vector_size], name="encoder{0}".format(i)))
    ...
    def predict(self, session, user_utterance, system_utterance=()):
        ...
        for i in range(encoder_size):
            input = [f_user[i]]  # batch_size = 1 vector
            em_vec = self.data_processor.embed(input)  # embed batch and make batch size x vector size matrix
