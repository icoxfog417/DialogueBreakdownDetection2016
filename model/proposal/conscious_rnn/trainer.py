...
class ConsciousRNNTrainer():
    ...
    def train(self, session, utterance_pairs, labels, check_interval=-1, max_iteration=-1):
        ...
        dp = ConsciousRNNDataProcessor(self.buckets)

        loss_hist = []
        hist_max = 5
        iteration = 0
        for bucket_id, encoder_inputs, decoder_inputs, weights in dp.batch_iter(utterance_pairs, labels, self.batch_size):
