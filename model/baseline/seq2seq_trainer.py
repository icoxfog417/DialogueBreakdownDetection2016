...
class Seq2SeqTrainer():
    ...
    def __init__(self, model, buckets, batch_size, train_dir=""):
        self.model = model
        ...
        for i in range(self.buckets[-1][0]):  # last bucket is the biggest one.
            self.encoder_inputs.append(tf.placeholder(tf.int32, shape=[self.batch_size], name="encoder{0}".format(i)))
    ...
    def train(self, session, utterance_pairs, check_interval=-1, max_iteration=-1):
        if self.optimizer is None:
            raise Exception("Optimizer is not set yet. Please call set_optimizer.")
        
        dp = Seq2SeqDataProcessor(self.buckets)

        loss_hist = []
        hist_max = 5
        iteration = 0
        for bucket_id, encoder_inputs, decoder_inputs, weights in dp.batch_iter(utterance_pairs, self.batch_size):
