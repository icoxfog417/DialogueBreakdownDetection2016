...
#TEST_DIR = os.path.join(os.path.dirname(__file__), "../../data/dialog_log/2016/evaluation/IRS")
GET_PATH = lambda x: os.path.join(os.path.dirname(__file__), "./store/" + x)

TARGET_PATH = GET_PATH("vector_rnn_target.txt")
TRAIN_PATH = GET_PATH("vector_rnn_target_train.txt")
TEST_PATH = GET_PATH("vector_rnn_test_target.txt")
VECTOR_PATH = GET_PATH("model_tokened.vec")
TRAIN_DIR = GET_PATH("training")
SUBMIT_DIR = GET_PATH("submit")

# todo: have to read from setting file
_vocab_size_ = 10000  # set max
...
def train():
    ...
    vocab_vectors = reader.user_loader.load_vocab_vectors()
    labels = reader.get_labels()

    print("begin training.")
    model = make_model(user_vocab, system_vocab, vocab_vectors)
    trainer = VectorRNNTrainer(model, _buckets_, _batch_size_, vocab_vectors, TRAIN_DIR)
...
def predict():
    ...
    model = make_model(analyzer.user_vocab, analyzer.system_vocab, vocab_vectors)
    model_if = VectoredRNNInterface(model, _buckets_, vocab_vectors, model_path=TRAIN_DIR)
...
def train_detector(neural_net=False, ignore_t=False):
    ...
    with tf.Session() as sess:
        ...
        if not neural_net:
            ...
            train_labels = [t.get_prob_vector() for t in train_t]
    ...
    analyzer.dump_result(os.path.dirname(TARGET_PATH), test_x, test_t, y, probs, "vector_rnn_train")
...
def predict_detector(neural_net=False, ignore_t=False, submit=False):
    ...
    analyzer.dump_result(os.path.dirname(TARGET_PATH), analyzer.dataset, analyzer.labels, y, probs, "vector_rnn_test")
